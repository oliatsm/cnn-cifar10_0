...
void pool_forward(float * restrict X, PoolLayer * l,float * restrict Y){
    printf("Pool: (%d,%d,%d)->(%d,%d,%d)\n",l->in_width,l->in_height,l->in_depth,
            l->out_width,l->out_height,l->out_depth);
    printf("    F:%d S:%d,P:%d\n",l->pool_width,l->stride,l->padding);
    for(int m=0;m<l->out_depth;m++){
        int x_j=-l->padding;
