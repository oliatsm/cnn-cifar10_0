...
// #pragma acc update self(O11[0:NUM_IMAGES][0:L11->out_size])
    ...
    #pragma acc enter data create(predictions[0:NUM_IMAGES])

#pragma acc parallel loop present(predictions, O11,L11) gang
    for (int n = 0; n < NUM_IMAGES; n++) {
        int class_o = 0;
        float max_o = -INFINITY;
        #pragma acc loop reduction(max:max_o,class_o) 
        for (int i = 0; i < L11->out_depth; i++) {
            if (max_o < O11[n][i]) {
                class_o = i;
                max_o = O11[n][i];
            }
    ...
    //Network Accuracy
    int correct_label = 0;
    #pragma acc parallel loop copyin(labels[0:NUM_IMAGES]) present(predictions) reduction(+:correct_label) //gang vector vector_length(32)
    for (int n = 0; n < NUM_IMAGES; n++) {
    ...
    #pragma acc exit data copyout(predictions[0:NUM_IMAGES])
