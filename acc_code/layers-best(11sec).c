...
#pragma acc parallel loop present(l,X)  gang vector_length(32)
...
void conv_forward(float* restrict X, Conv_Layer* l, float* restrict Y) {
  ...
  #pragma acc parallel loop present(l,Y) gang worker collapse(3) vector_length(32)
...
void fc_forward(float* restrict X, FC_Layer* l, float* restrict Y) {
  ...
  #pragma acc parallel loop present(l,X,Y) gang worker vector_length(32)
  for (int i = 0; i < l->out_depth; i++) {
    // Calculate dot product of input and weights
    float sum = 0.0f;
    #pragma loop reduction(+:sum) vector
...
void softmax_forward(float* restrict X, Softmax_Layer* l, float* restrict Y) {
  ...
  #pragma acc data present(X,l,Y) create(max,total) cache(X[0:10])
  {
  // Compute max activation
  #pragma acc parallel loop reduction(max:max) vector vector_length(32)
  for (int i = 0; i < l->out_depth; i++) {
    if (X[i] > max) {
      max = X[i];
    }
  ...
  #pragma acc parallel loop reduction(+:total) vector vector_length(32)
  for (int i = 0; i < l->out_depth; i++) {
    float e = exp(X[i] - max);
    total += e;
    l->likelihoods[i] = e;
  ...
  #pragma acc parallel loop vector vector_length(32)
  for (int i = 0; i < l->out_depth; i++) {
