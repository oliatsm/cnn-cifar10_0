...
void conv_forward(float* X, Conv_Layer* l, float* Y) {
...
void relu_forward(float* X, ReLU_Layer* l, float* Y) {
...
void pool_forward(float* X, Pool_Layer* l, float* Y) {
  // For each output feature map
  for (int m = 0; m < l->out_depth; m++) {
    for (int j = 0; j < l->out_height; j++) {
      for (int i = 0; i < l->out_width; i++) {
...
void fc_forward(float* X, FC_Layer* l, float* Y) {
...
void softmax_forward(float* X, Softmax_Layer* l, float* Y) {
